  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Adventure Calendar</h1>
                ...
                <div className="pt-2 border-t">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="text-lg">
                      {MOOD_RATINGS.find(r => r.value === Math.round(
                        monthEntries.reduce((sum, e) => sum + e.moodRating, 0) / monthEntries.length
                      ))?.emoji}
                    </span>
                    <span className="font-medium">
                      {(monthEntries.reduce((sum, e) => sum + e.moodRating, 0) / monthEntries.length).toFixed(1)}/5
                    </span>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-3">
                  {monthEntries
                    ...
                    .map((entry) => {
                      const moodData = MOOD_RATINGS.find(r => r.value === entry.moodRating);
                      return (
                        <div key={entry.id} className="flex items-start space-x-3 p-2 rounded-lg hover:bg-muted/50 cursor-pointer">
      ...
      {selectedDate && selectedEntries.length > 0 && (
        <Card className="family-card">
          <CardHeader>
            <CardTitle>
              Adventures on {new Date(selectedDate).toLocaleDateString('en-US', { 
                weekday: 'long', 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
      ...
      {selectedDate && selectedEntries.length === 0 && (
        <Card className="family-card">
          <CardContent className="text-center py-8">
            ...
            <h3 className="text-lg font-semibold mb-2">No Adventures This Day</h3>
            <p className="text-muted-foreground">
              No family adventures were recorded on {new Date(selectedDate).toLocaleDateString()}
            </p>
          </CardContent>
        </Card>
      )}
