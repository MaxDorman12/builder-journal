...
export function FamilyStats({ showAll = false, onViewAll }: FamilyStatsProps) {
  ...
  const calculateStats = () => {
    try {
      ...
      const totalPhotos = entries.reduce((sum, entry) => sum + entry.images.length, 0);
      ...
      const averageMood = entries.length > 0 
        ? Math.round(entries.reduce((sum, entry) => sum + entry.moodRating, 0) / entries.length)
        : 0;
      ...
      const paidActivitiesCount = entries.filter(e => e.isPaidActivity).length;
      const freeActivitiesCount = entries.filter(e => !e.isPaidActivity).length;
      ...
      for (let i = 0; i <= currentMonth; i++) {
        const monthEntries = entries.filter(entry => {
          ...
          return entryDate.getFullYear() === currentYear && entryDate.getMonth() === (currentMonth - i);
        });
      ...
      const bestRatedEntry = entries.length > 0 
        ? entries.reduce((best, entry) => 
            entry.moodRating > best.rating ? { title: entry.title, rating: entry.moodRating } : best,
            { title: entries[0].title, rating: entries[0].moodRating }
          )
        : null;

      const mostLovedEntry = entries.length > 0
        ? entries.reduce((most, entry) =>
            entry.likes > most.likes ? { title: entry.title, likes: entry.likes } : most,
      ...
      entries.forEach(entry => {
        adventuresByMood[entry.moodRating] = (adventuresByMood[entry.moodRating] || 0) + 1;
      });
  ...
  const basicStats = [
    ...
    {
      icon: () => <span className="text-2xl bouncy">{getMoodEmoji(stats.averageMood)}</span>,
  ...
  return (
    <div className="space-y-6">
      {/* Main Stats Grid */}
      <div className={`grid grid-cols-2 ${showAll ? 'lg:grid-cols-4' : 'md:grid-cols-4'} gap-3 md:gap-4`}>
          ...
          return (
            <Card key={index} className={`pastel-stat-card ${stat.color}`}>
              <CardContent className="flex items-center space-x-3 p-4">
                ...
                <div>
                  <p className={`text-lg font-bold ${stat.textColor.replace('text-', 'text-').replace('-700', '-800')}`}>
                    {stat.value}
                  </p>
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <Badge variant="secondary">{stats.mostActiveAuthor.name} ({stats.mostActiveAuthor.count})</Badge>
                  </div>
                  <div className="flex justify-between">
                    <span>Favorite Adventure Type:</span>
                    <Badge variant="secondary">{getAreaTypeLabel(stats.favoriteAreaType.type)}</Badge>
                  </div>
                  {stats.bestRatedEntry && (
                    <div className="flex justify-between">
                      <span>Best Rated Adventure:</span>
                      <Badge variant="secondary">{getMoodEmoji(stats.bestRatedEntry.rating)} {stats.bestRatedEntry.rating}/5</Badge>
                    </div>
                  )}
                  {stats.mostLovedEntry && (
                    <div className="flex justify-between">
                      <span>Most Loved Entry:</span>
                      <Badge variant="secondary">❤️ {stats.mostLovedEntry.likes} likes</Badge>
                    </div>
                  )}
                </div>
              </CardContent>
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2">
                  {Object.entries(stats.adventuresByMood)
                    ...
                    .map(([mood, count]) => {
                      const moodData = MOOD_RATINGS.find(r => r.value === parseInt(mood));
                      return (
          ...
          <Card className="family-card">
            <CardContent className="p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-orange-600">{stats.busyLocationsCount}</p>
                  <p className="text-muted-foreground">🏃 Busy Places</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-purple-600">{stats.totalEntries - stats.busyLocationsCount}</p>
