...
export class HybridStorage {
  ...
  static debug = {
    ...
    // Enhanced debugging for map pin sync issues
    compareMapPins: async () => {
    ...
    // Force sync all map pins from Supabase to local
    forceMapPinSync: async () => {
      console.log("🔄 FORCE MAP PIN SYNC: Starting...");
      const remote = await SupabaseDatabase.getMapPins();
      console.log(`🔄 FORCE SYNC: Found ${remote.length} pins in Supabase`);
      ...
      // Save all remote pins to local storage
      remote.forEach((pin) => {
        console.log(`🔄 FORCE SYNC: Saving pin ${pin.title} (${pin.id})`);
        LocalStorage.saveMapPin(pin);
      });
      ...
      this.notifyListeners();
      console.log("✅ FORCE MAP PIN SYNC: Completed");
      return LocalStorage.getMapPins();
    },
  };
}
